...
export const importEcsSecrets = (scope: Construct, secretsToImport: FargateServiceSecret[]) => {
    const secrets: { [key: string]: ecs.Secret } = {};

    secretsToImport.forEach((secret) => {
        const secretsManagerSecret = secretsmanager.Secret.fromSecretPartialArn(scope, `${secret.taskDefSecretName}Secret`, `arn:aws:secretsmanager:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:secret:${secret.secretsManagerSecretName}`);
