...
export function getSg(scope: Construct, props?: ICreateMockSgProps) {
    ...
    const mockVpc = new ec2.Vpc(scope, `MockSgVpc-${Math.random().toString(36).substring(4)}`);

    return new ec2.SecurityGroup(scope, `MockSg-${Math.random().toString(36).substring(4)}`, {
